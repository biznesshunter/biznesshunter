...
function Get-BusinessIdentityTokens {
    ...
    $stopWords = @(
        ...
        'everyday','tasks','video','social','network','members','ai','agents','windows','nle','openfx','proxy','editing','subs','vtuber'
    )
...
function Test-BusinessIdentity {
    ...
    $name = $BusinessName.ToLower().Trim()
    $titleText = $Title.ToLower()
    $urlText = $Url.ToLower()

    if ($titleText -match [regex]::Escape($name)) {
        return $true
    }

    if ($urlText -match [regex]::Escape($name)) {
        return $true
    }
    ...
    foreach ($token in $tokens) {
        if ($titleText -match "\b$([regex]::Escape($token))\b") {
            $matchedTitle++
        }

        if ($urlText -match [regex]::Escape($token)) {
            $matchedUrl++
        }
    }

    if ($tokens.Count -ge 2) {
        return ($matchedTitle -ge 1 -or $matchedUrl -ge 1)
    }
...
foreach ($item in $queue) {
    ...
    Write-Host "IDENTITY MATCH : $($identityResults.Count) / $($uniqueResults.Count)" -ForegroundColor Magenta
