...
$tractionPatterns = @(
    ...
    '(?i)\b\d[\d,.]*\s*(million|m|k|thousand)?\s*(customers|clients|users|subscribers|members)\b',
    '(?i)\b\d[\d,.]*\s*(active|paying)\s*(users|customers|clients|subscribers|members)\b',
    '(?i)\b(over|more than|above)\s+\d[\d,.]*\s*(customers|clients|users|downloads|orders|bookings)\b',
    '(?i)\bused by\s+\d[\d,.]*\s*(companies|businesses|teams|users|customers|clients)?\b',
    '(?i)\bserves\s+\d[\d,.]*\s*(customers|clients|users|businesses|companies)\b',
    ...
    '(?i)\b\d[\d,.]*\s*(users|customers|clients|members|subscribers)\s+(use|using|pay|paying)\b',

    # Commercial proof expressed as numbers
    '(?i)\b\d[\d,.]*\s*(million|m|k|thousand)?\s*(in revenue|revenue|sales|arr|mrr)\b',
    '(?i)\b(reached|hit|generated|made|earned)\s+(\$|€|£)?\s?\d[\d,.]*\s*(million|m|k|thousand)?\b'
)
