...
$results = foreach ($item in $items) {
    ...
    #
    # Un produit détecté n'est pas automatiquement un business.
    # On recherche des signaux indiquant qu'il existe une offre
    # commerciale réelle ou un modèle monétisable.
    ...
    foreach ($pattern in $commercialPatterns) {
        ...
        if ($lower -match $regex) {
            $commercialSignals += 1
            $signals += "COMMERCIAL:$pattern"
        }
    }

    # Un produit clairement identifié vaut déjà quelque chose,
    # mais il faut distinguer produit et business.
    if ($isProduct) {
        $score += 3
    }

    # Preuve commerciale explicite
    if ($commercialSignals -ge 1) {
        $score += 2
        $signals += "COMMERCIAL_SIGNAL"
    }

    # Plusieurs signaux commerciaux = meilleure confiance
    if ($commercialSignals -ge 2) {
        $score += 2
        $signals += "COMMERCIAL_VALIDATED"
    }
    if ($score -ge 1) {
