...
function Get-BusinessIdentityTokens {
    ...
    # Remove price / parenthetical noise
    $clean = $clean -replace '\([^)]*\)', ' '
...
foreach ($item in $queue) {
    ...
    # ==============================================
    # V25-A — BUSINESS IDENTITY FILTER
    # ==============================================
    ...
    Write-Host "IDENTITY         : $(@($identityResults).Count)"`r`n    Write-Host "COMMERCIAL       : $commercialCount"
...
$outputFile = ".\validation_results_v24.json"
...
Write-Host " V24 COMPLETE" -ForegroundColor Green
Write-Host "======================================" -ForegroundColor Green
Write-Host ""
...
function Test-BusinessIdentity {
    ...
    # Single-word brands need stronger identity confirmation.
    if ($tokens.Count -eq 1) {
        ...
        if ($titleLower -match [regex]::Escape($token)) {
            return $true
        }

        if ($urlLower -match [regex]::Escape($token)) {
            return $true
        }
    ...
    # Multi-token business names:
    # require at least 50% of meaningful tokens.
